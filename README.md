...
| **Финальная модель** | оттюненный CatBoost + drift-aware калибровка по возрасту магазинаов |
| `Мой результат` **Val MAPE** | 14.69% (TARGET_2) |
...
- **Сокомандник сдал основное решение** (CatBoost (TARGET_2) / CatBoost + LightGBM бленд (TARGET_1) + глобальный correction для компенсации дрейфа, ~14.24% на лидерборде). 
